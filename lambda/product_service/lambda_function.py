...
def get_flash_sale_products():
    ...
    result = table.scan(
        FilterExpression="is_flash_sale = :t AND drop_end_time > :now",
...
def configure_flash_sale(product_id, body):
    logger.info("Configuring flash sale", product_id=product_id)
    existing = table.get_item(Key={"product_id": product_id}).get("Item")
    if not existing:
        return response(404, {"message": "Product not found"})
    allowed = ["is_flash_sale", "drop_end_time", "flash_label", "stock"]
    fields = {k: v for k, v in body.items() if k in allowed}
    if not fields:
        return response(400, {"message": "No valid fields"})
    update_expression = "SET " + ", ".join(f"#f_{k} = :{k}" for k in fields)
    expression_names  = {f"#f_{k}": k for k in fields}
    expression_values = {f":{k}": v for k, v in fields.items()}
    table.update_item(
        Key={"product_id": product_id},
        UpdateExpression=update_expression,
        ExpressionAttributeNames=expression_names,
    ...
    logger.info("Flash sale configured", product_id=product_id)
    return response(200, {"message": "Flash sale configured", "product_id": product_id})


def lambda_handler(event, context):
    logger.info("Request received", route=event.get("routeKey"), request_id=context.aws_request_id)
    ...
    try:
        ...
        elif route == "GET /products/drops":
            return get_flash_sale_products()
        elif route == "GET /products/{product_id}":
            return get_product(product_id)
        elif route == "POST /products":
            return create_product(body)
        elif route == "PUT /products/{product_id}/flash":
            return configure_flash_sale(product_id, body)
    ...
    route = event.get("routeKey", "")
    logger.info("RAW ROUTE", raw=route)  # ADD THIS
    route = normalize_route(route)
    logger.info("NORMALIZED ROUTE", normalized=route)  # ADD THIS
